...
ap_message GCS_MAVLINK::mavlink_id_to_ap_message_id(const uint32_t mavlink_id) const
{
	...
	static const struct {
		...
		ap_message msg_id;
			...
			{ MAVLINK_MSG_ID_PERMISSION_ARTIFACT, 	MSG_PERMISSION},
 ...
 void GCS_MAVLINK::send_heartbeat() const
{
 ...
 /*
  * handle permission artifact verification request
  */
 MAV_RESULT GCS_MAVLINK::handle_permission_artifact()
 {

#ifdef HAL_IS_REGISTERED_FLIGHT_MODULE
	 AP_Security &security = AP::security();
 ...
 MAV_RESULT GCS_MAVLINK::handle_command_long_packet(const mavlink_command_long_t &packet)
 {
	 ...
	 case MAV_CMD_PERMISSION:
		 result = handle_permission_artifact();
		 break;
...
break;
	 ...
	 case MSG_PERMISSION:
		 CHECK_PAYLOAD_SIZE(PERMISSION_ARTIFACT);
		 AP::security().send_mavlink_status(chan);
		 break;
