...
void KeyManager::_sign_hash(void* _key_mgr){
	...
	if (ret > 0) {
		...
		digest_len = 32 + MAX_DER_DIGEST_SZ;
		digest_buf = (byte*)malloc(digest_len);
		if (digest_buf) {
			ret = wc_EncodeSignature(digest_buf, key_mgr->_hashed_data, 32, oid);
			if (ret > 0) {
				digest_len = ret;
				ret = 0;
			}
	...
	byte* signature;
	sig_len = wc_SignatureGetSize(WC_SIGNATURE_TYPE_RSA_W_ENC, &key_mgr->ap_key, sizeof(key_mgr->ap_key));
	signature = (byte*)malloc(sig_len);

	if(ret == 0){
		ret = wc_RsaSSL_Sign(digest_buf, digest_len, signature, sig_len, &key_mgr->ap_key, &rng);
	}
	if(ret == 256){
		key_mgr->log_signed = true;
		key_mgr->signed_log = signature;
	}
}
...
int KeyManager::sign_hash_with_key(uint8_t* hashed_data, uint16_t hashed_data_len, uint8_t* signature){
	...
	int ret =0;
	_hashed_data = hashed_data;

	if(ret == 0){
		thread_create_alloc(THD_WORKING_AREA_SIZE(4096),
				"LOGGEN",
				APM_IO_PRIORITY-1,           /* Initial priority.    */
				KeyManager::_sign_hash,    /* Thread function.     */
				this);
	}
	if(ret != 0){
		return 0;
